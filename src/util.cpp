...
const char * const BITCOIN_CONF_FILENAME = "sscoin.conf";
const char * const BITCOIN_PID_FILENAME = "sscoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "sscoin" is a composite category enabling all Sscoin-related debug output
            if(ptrCategory->count(string("sscoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SscoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SscoinCore
    // Mac: ~/Library/Application Support/SscoinCore
    // Unix: ~/.sscoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SscoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SscoinCore";
#else
    // Unix
    return pathRet / ".sscoincore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty sscoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override sscoin.conf
